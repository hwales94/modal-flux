...
app = modal.App(
    "imagenet-download",
    image=(modal.Image.debian_slim().pip_install("webdataset", "huggingface_hub[cli]", "hf_transfer").env({
...
def download_imagenet(split: str, file: str, start_shard: int):
    ...
    os.system(f"huggingface-cli download ILSVRC/imagenet-1k data/{file} --repo-type dataset --local-dir /data")
    ...
    with wds.ShardWriter(f"/data/{split}/%06d.tar", maxcount=10000, start_shard=start_shard) as sink:
